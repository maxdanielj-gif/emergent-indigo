...
async function generateAndSendProactiveMessage(
  ...
  const { chatHistory, aiProfile, userProfile, anthropicApiKey: clientKey, pushSubscription, timeZone, isAmbient, userId } = userData;
...
app.post("/api/voices", express.json(), async (req, res) => {
  ...
  try {
    ...
    if (!response.ok) throw new Error(`Async Voices error: ${response.statusText}`);
...
app.post("/api/memory-extract", async (req, res) => {
  ...
  try {
    const client = getAnthropicClient(clientKey);
    const prompt = `Extract any new, significant long-term fact about ${userProfile.name} from this interaction that ${aiProfile.name} should remember.

${userProfile.name}: ${userMsg}
${aiProfile.name}: ${aiMsg}

Already known:
${(existingMemories || []).map((m: any) => m.content).join("; ")}

Write one concise sentence from ${aiProfile.name}'s perspective if there is a new fact worth remembering. Otherwise write exactly NOTHING.`;
